...
import gwent.cards.abilities.{Ability, DoublePower}

class Radovid extends RangedCombatUnit("King Radovid V of Redania", 6){
   ...
   */
  override def canEqual(that: Any): Boolean = that.isInstanceOf[Radovid]
  ...
  override def hashCode(): Int = {
    val primo = 31
    var total = 1
    total = primo * total + classOf[Radovid].##
    total = primo * total + name.##
    total = primo * total + basePower
    total
  }

  override def toString: String = s"Radovid(basePower=$getBasePower, currentPower=$currentPower, ability=DoublePower)"
