...
class Foltest extends CloseCombatUnit("King Foltest of Temeria", 6){
   ...
   */
  override def canEqual(that: Any): Boolean = that.isInstanceOf[Foltest]
  ...
  override def hashCode(): Int = {
    val primo = 31
    var total = 1
    total = primo * total + classOf[Foltest].##
    total = primo * total + name.##
    total = primo * total + basePower
    total
  }

  override def toString: String = s"Foltest(basePower=$getBasePower, currentPower=$currentPower, ability=DoublePower)"
